...
export function getProductCategory(category, selector) {
  if (category === "all") return products;
    ...
    return products.filter(
      (item) =>
        item.price >= Number(selectors[0]) && item.price <= Number(selectors[1])
    );
...
function importAll(r) {
  r.keys().forEach((item) =>
    products[Number(item.slice(5, 7))]["images"].push(r(item))
  );
}
importAll(require.context("./imgs", false, /\.(png|jpe?g|svg|webp)$/));
